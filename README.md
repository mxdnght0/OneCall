...
Выполняет функцию `f` только один раз для каждого значения `key`. Если для этого `key` функция уже выполняется, то остальные вызовы будут ждать её завершения и получат тот же результат.
